...
geometry_params1 = [0.18440598, 0, 0.39424837]  # r1, r2, distance between centres of two cylinders
normal_params1 = [0.92150683, 0.92150683, 0.75147275, hs_dbr, hs_SiO2_dbr] #L1,L2,h,h_Sidbr,h_Sio2
...
geometry_func2 = get_epgrid_double_cylinder_d_new

'''Geometry 2'''

geometry_params2 = [0.17240458, 0, 0.4907464] 
normal_params2 = [1.09685312, 1.09685312, 0.35899229, hs_dbr, hs_SiO2_dbr]




'''Gives you phase as a function of wavelengths'''
phis2,r_amp2 = compute_phase(geometry_func2, geometry_params2, lambdas, normal_params2, DBR_PAIRS)
...
interp_phi2 = interp1d(omega_sorted, phis2_sorted, kind='cubic', 
                       bounds_error=False, fill_value=(phis2_sorted[0], phis2_sorted[-1]))

phi1_uniform = interp_phi1(omega_uniform)
phi2_uniform = interp_phi2(omega_uniform)
...
Eout2 = Ein * np.exp(1j*phi2_uniform)
...
Eout2_t = np.fft.fftshift(
    np.fft.ifft(np.fft.ifftshift(Eout2))
) * Nw * df

# Intensities
Iin = np.abs(Ein_t)**2
Iout1 = np.abs(Eout1_t)**2
Iout2 = np.abs(Eout2_t)**2


Ein_energy = np.trapz(Iin, t)
Eout_energy = np.trapz(Iout2, t)

print("Energy ratio (should be ~1):", Eout_energy / Ein_energy)
...
delay2_fs = (t_2_peak - t_in_peak) * 1e15
delta12_fs = (t_2_peak - t_1_peak) * 1e15

print(f"Geometry 1 delay: {delay1_fs:.2f} fs")
print(f"Geometry 2 delay: {delay2_fs:.2f} fs")
print(f"Difference (G2 - G1): {delta12_fs:.2f} fs")
...
plt.plot(t*1e15, Iout1/scale, label="Geometry 1 (slope = 140)")
plt.plot(t*1e15, Iout2/scale, label="Geometry 2 (slope = 15)")
plt.xlim(-1000,1000)
plt.xlabel("Time (fs)")
...
plt.plot(t*1e15, Iin/scale, 'k--', linewidth=1.5, label="Input")
plt.plot(t*1e15, Iout1/scale, linewidth=2, label="Geometry 1")
plt.plot(t*1e15, Iout2/scale, linewidth=2, label="Geometry 2")

# Vertical lines at peaks
plt.axvline(t_1_peak*1e15, linestyle=':', linewidth=1)
plt.axvline(t_2_peak*1e15, linestyle=':', linewidth=1)
...
t_com_out2 = np.sum(t * Iout2) / np.sum(Iout2)

###print(f"CoM Delay Geometry 1: {(t_com_out1 - t_com_in)*1e15:.3f} fs")
print(f"CoM Delay Geometry 2: {(t_com_out2 - t_com_in)*1e15:.3f} fs")
...
dphi2 = np.gradient(phi2_uniform, omega_uniform)
...
print("Group delay at center (Geometry 2):", tau2_fs[idx0], "fs")
...
ax.plot(omega_uniform, phi1_uniform, linewidth=2, label="Geometry 1")
ax.plot(omega_uniform, phi2_uniform, linewidth=2, label="Geometry 2")
...
phi2_lin  = phi2_uniform[mask_linear]
...
coef2 = np.polyfit(omega_lin, phi2_lin, 1)
...
tau2_linear_fs = slope2 * 1e15

print("Linear-fit Group Delay (Geometry 1):", tau1_linear_fs, "fs")
print("Linear-fit Group Delay (Geometry 2):", tau2_linear_fs, "fs")
...
tau2_eff, GDD2_eff = compute_dispersion_metrics(phi2_uniform, omega_uniform, Ein)

tau1_eff_fs = tau1_eff * 1e15
tau2_eff_fs = tau2_eff * 1e15

GDD1_eff_fs2 = GDD1_eff * (1e15)**2
GDD2_eff_fs2 = GDD2_eff * (1e15)**2

print("Effective GD G1 (fs):", tau1_eff_fs)
print("Effective GD G2 (fs):", tau2_eff_fs)
print("Effective GDD G1 (fs^2):", GDD1_eff_fs2)
print("Effective GDD G2 (fs^2):", GDD2_eff_fs2)
...
d2phi2 = np.gradient(dphi2, omega_uniform)

tau1_fs_full = dphi1 * 1e15
tau2_fs_full = dphi2 * 1e15

GDD1_fs2_full = d2phi1 * (1e15)**2
GDD2_fs2_full = d2phi2 * (1e15)**2

# Restrict to linear window
tau1_fs_lin = tau1_fs_full[mask_linear]
tau2_fs_lin = tau2_fs_full[mask_linear]

GDD1_fs2_lin = GDD1_fs2_full[mask_linear]
GDD2_fs2_lin = GDD2_fs2_full[mask_linear]
...
# (a) Spectral Phase
# ----------------------------------------------------------
axs[0].plot(omega_lin, phi1_lin, linewidth=2, label="Geometry 1")
axs[0].plot(omega_lin, phi2_lin, linewidth=2, label="Geometry 2")

axs[0].plot(omega_lin, np.polyval(coef1, omega_lin), '--', linewidth=1)
axs[0].plot(omega_lin, np.polyval(coef2, omega_lin), '--', linewidth=1)

axs[0].set_ylabel("Phase φ(ω) (rad)")
axs[0].set_title("(a) Spectral Phase (1.50–1.52 µm)")
...
# (b) Group Delay
# ----------------------------------------------------------
axs[1].plot(omega_lin, tau1_fs_lin, linewidth=2, label="Geometry 1")
axs[1].plot(omega_lin, tau2_fs_lin, linewidth=2, label="Geometry 2")

axs[1].axhline(tau1_eff_fs, linestyle='--', linewidth=1,
               label=f"Effective GD G1 = {tau1_eff_fs:.1f} fs")

axs[1].axhline(tau2_eff_fs, linestyle='--', linewidth=1,
               label=f"Effective GD G2 = {tau2_eff_fs:.1f} fs")
...
# (c) GDD
# ----------------------------------------------------------
axs[2].plot(omega_lin, GDD1_fs2_lin, linewidth=2, label="Geometry 1")
axs[2].plot(omega_lin, GDD2_fs2_lin, linewidth=2, label="Geometry 2")
...
centroid_2   = compute_centroid(t, Iout2)

print("Centroid delay G1 (fs):", (centroid_1 - centroid_in)*1e15)
print("Centroid delay G2 (fs):", (centroid_2 - centroid_in)*1e15)
...
skew_2   = compute_skewness(t, Iout2)

print("Skewness in :", skew_in)
print("Skewness G1:", skew_1)
print("Skewness G2:", skew_2)
...
phase_type = "Linear"            # Linear / Quadratic / Cubic
geometry_type = "Double Cylinder"
geometry_id = "G1"
pulse_bandwidth_nm = 10          # Your bandwidth in nm
...
dphi = np.gradient(phi2_uniform, omega_uniform)
tau_fs = dphi * 1e15
...
centroid_in  = compute_centroid(t, Iin)
centroid_out = compute_centroid(t, Iout2)
...
rms_in  = compute_rms_width(t, Iin)
rms_out = compute_rms_width(t, Iout2)
...
skewness = compute_skewness(t, Iout2)
